...
do
        if [ $distribution = xenial-security ]; then
                sudo aptly publish snapshot -architectures="amd64,source" -component=,,, -gpg-key="300F9C58" -distribution="$l" security_ubuntu_com_${distribution}_main-${date} security_ubuntu_com_${distribution}_restricted-${date} security_ubuntu_com_${distribution}_universe-${date} security_ubuntu_com_${distribution}_multiverse-${date} ubuntu${date}
        else
